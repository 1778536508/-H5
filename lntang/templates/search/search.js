...
//		console.log(getCategoryTextById(13));
		...
		$('#btn').click(function(){
			...
			if(cate != 0 && cate != null && cate != ''){
				ajaxData(val, offnum, lim, num, cate);
			}else{
				ajaxData(val, offnum, lim, num);
			}
			
		})
			

		function ajaxData(val, off, lim, num, cate) {
			...
			//参数
			var condition = { "keyword": val, "offset": off, "limit": lim, "category":cate};
			$.ajax({
				type: 'post',
