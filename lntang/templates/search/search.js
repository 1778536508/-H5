...
define(["text!search/search.tpl"], function(testTpl) {
	...
	setTimeout(function(){
		...
		//搜索
		$('#sou').click(function() {
//			arr_Cate.splice(0,arr_Cate.length);
//			arr_Code.splice(0,arr_Code.length);
		...
		function ajaxData(val, off, lim, num, cate, code, type) {
			...
			if(cate != undefined && cate != 0 && code != undefined && code != "" && type != null){
				condition = { "keyword": val, "offset": off, "limit": lim, "category":cate, "area":code, "type":type};
			}else{
				condition = { "keyword": val, "offset": off, "limit": lim, "type":type};
				if(cate != undefined && cate != 0 || code == undefined && code == ""){
					condition = { "keyword": val, "offset": off, "limit": lim, "category":cate, "type":type};
				}
				if(cate == undefined && cate == 0 || code != undefined && code != ""){
					condition = { "keyword": val, "offset": off, "limit": lim, "area":code, "type":type};
				}
			}
