...
define(["text!search/search.tpl"], function(testTpl) {
	...
	function _onGetData() {
		...
		setTimeout(function(){
			...
			function resetClass(){
				$('#list_type li:first-child').addClass('type').siblings().removeClass('type');
				$('#list_menu_content li:first-child').addClass('blue_one_Title').siblings().removeClass('blue_one_Title');
				$('#list_reg li:first-child').addClass('blue_one_Title').siblings().removeClass('blue_one_Title');
				$('#list_menu_span').text('全部');
				$('.list_menu_one').text('所有分类');
				$('#list_menu_region').text('全球');
			}
		},10);	
	...
	//			console.log(type == null ? 1 : 2)
				...
				if(val == "" && (cate == "undefined" || cate == null || cate == undefined || cate == 0) && (code == "undefined" || code == undefined || code == "" || code == null) && (type == null || type == "null" || type == undefined)){
					$('#paging').css('display','none');
					...
					success: function(obj) {	
						...
						if(condition.type != 1){
							...
							$(document).ready(function() {
								var conImg = document.getElementsByClassName('conImg');
								for(var i=0; i<conImg.length; i++){
									conImg[i].onerror = function(){
										this.setAttribute('src','assets/images/headsmall.png');
									}
								}
							});
			...
			//search最多显示h3
			function forma_Load(){
			  var forma_h3 = $('.forma_duce_float');
			  for(var i = 0; i<forma_h3.length; i++){
			  		var str = $(forma_h3[i]).children('h3').html();
					var s = str;
					if(str.length>10){
						s = str.substring(0,10)+'...';
					}
					$(forma_h3[i]).children('h3').html(s);
			  }
			}
			...
			//搜索喜欢
		    $('#forma_pics').on('click','a',function(){
		    	var num = parseInt($(this).children('span').text());
		    	
		    	if($(this).children('img').eq(1).css('display') == 'none'){
